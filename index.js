...
class App extends Component {
  ...
  componentDidMount() {
    try {
      this.ai = new GoogleGenAI({ apiKey: 'AIzaSyDwHLcHAExY8daSAxgtJThOslTzoRB-seY' });
    } catch (e) {
      console.error(e);
  ...
  handleProductSelect = (p) => {
    const apparelProducts = ['t-shirt', 'pants', 'cap', 'bag mockup', 'shoes', 'undergarments', 'sunglasses'];

    let newAiSubject = null;
  ...
  handleFileChange = (e, type) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (type === 'design') {
          this.setState({ designImage: reader.result });
        } else {
          this.setState({ customModelImage: reader.result });
        }
      };
  ...
  generateValidationHash = (deviceId) => {
  ...
  generateMockup = async () => {
    ...
    try {
      ...
      const tryOnProducts = ['shoes', 'undergarments', 'sunglasses'];
      ...
      if (modelType === 'custom' && customModelImage) {
        ...
        if (isTryOnProduct) {
          ...
          parts.push(customModelPart, designImagePart, { text: promptText });
            ...
            const apparelProducts = ['t-shirt', 'pants', 'cap', 'bag mockup'];
      ...
      console.error(e);
      this.setState({
        error: `An error occurred during generation: ${e.message}`,
        isLoading: false,
      });
    }
  }
  ...
  render() {
    ...
    const productList = ['t-shirt', 'pants', 'cap', 'bag mockup', 'phone case', 'tablet ebook', 'car sticker', 'bike sticker', 'fridge magnet', 'big bottle', 'shoes', 'undergarments', 'sunglasses'];
    const productsWithModelSelection = ['t-shirt', 'pants', 'cap', 'bag mockup', 'shoes', 'undergarments', 'sunglasses'];
    const stickerProducts = ['car sticker', 'bike sticker'];
    ...
    const carSubjectKeys = Object.keys(carSubTypes);
    const isCarSubjectSelected = aiModelSubject !== null && carSubjectKeys.includes(aiModelSubject);
    
    const vehicleSubjects = [...carSubjectKeys, 'bike', 'bus', 'train'];
                    ...
                    h('div', { className: 'button-group' },
                       Object.entries(carSubTypes).map(([key, value]) =>
                        h('button', {
                          ...
                          onClick: () => this.setState({ aiModelSubject: key })
                        }, value)
                      )
                    )
                  ),
            ...
            credits <= 0 && h('div', { className: 'pricing-message' },
              ...
              h('div', { className: 'credit-key-container' },
                 h('input', {
                   ...
                   onInput: (e) => this.setState({ creditKeyInput: e.target.value, keyStatusMessage: null })
                 }),
